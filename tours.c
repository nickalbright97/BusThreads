...
sem_t busEmpty, canStart, beltsFastened, startSing, fastenAlert, songsSung, availSeats, tourDone, shmAccess; // used
...
void *indy( void *ptr) {
    ...
    while (1 == 1) {
        ...
        printf("Indy     : New Tour. Declaring %d vacant seats\n", sBus->totalSeats); 
    ...
    printf("Indy     : Business is now closed. I did %2d tours today\n", numTours);
    return NULL;
}
...
void *tourist (void *ptr) {
    ...
    while (curTour < sBus->totalTrips) {
        ...
        printf("Tourist %ld: Tour # %d.  Going to shop for %d milliseconds.\n", tID, curTour + 1, randDur);
        sBus->inTown++;
        usleep(randDur);
